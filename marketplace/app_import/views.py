...
class ImportView(CategoryMixin, PageInfoMixin, LoginRequiredMixin, generic.FormView):
    ...
    def get_context_data(self, **kwargs):
        protocols = ImportProtocol.objects.active_tasks(
...
class TaskCheckView(LoginRequiredMixin, generic.View):
    def post(self, request):
        ...
        tasks = [
            ...
            for task in ImportProtocol.objects.tasks_results(
                user=request.user, tasks=uuids
            )
        ]
        return JsonResponse({"success": bool(tasks), "error": error, "tasks": tasks})
