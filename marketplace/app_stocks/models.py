...
class Stocks(models.Model):
    ...
    method = models.ForeignKey(
        ...
        verbose_name=_("Stock method"),
        on_delete=models.CASCADE,
        related_name="stock_method",
    )
    amount = models.DecimalField(_("Stock amount"), decimal_places=10, max_digits=18)
    index_min = models.DecimalField(
        _("Min stock index"), decimal_places=10, max_digits=18
    )
    index_max = models.DecimalField(
        _("Max stock index"), decimal_places=10, max_digits=18
    )
    ...
    def __str__(self):
        period = _("%(begin_date)s &ndash; %(end_date)s") % {
            "begin_date": date_format(
                self.since, format="SHORT_DATE_FORMAT", use_l10n=True
            ),
            "end_date": date_format(
                self.until, format="SHORT_DATE_FORMAT", use_l10n=True
            ),
        }
