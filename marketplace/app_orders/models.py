...
class DeliveryMethods(models.Model):
    ...
    price = models.DecimalField(
        _("Delivery price"), default=0, decimal_places=2, max_digits=19
    )
...
class Orders(models.Model):
    ...
    total_sum = models.DecimalField(
        _("Total sum"), default=0, decimal_places=2, max_digits=19
    )
    ...
    bank_account = models.CharField(
        _("Bank account for payment"), max_length=20, blank=True, null=True
    )
    ...
    def __str__(self):
        return _("%(begin_date)s &ndash; %(total_sum)s. %(pay_state)s.") % {
            "begin_date": date_format(
                self.date_time, format="SHORT_DATE_FORMAT", use_l10n=True
            ),
...
class OrderItems(models.Model):
    order = models.ForeignKey(
        Orders, verbose_name=_("Order"), on_delete=models.CASCADE, related_name="items"
    )
    ...
    good = models.ForeignKey(
        Goods, verbose_name=_("Good"), on_delete=models.CASCADE, related_name="good"
    )
